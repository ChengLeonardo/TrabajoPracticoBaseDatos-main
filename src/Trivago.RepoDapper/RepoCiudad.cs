...
public class RepoCiudad : RepoDapper, IRepoCiudad
{
    ...
    {
        string sql = "ListarCiudadPorPais";
        var resultado = _conexion.Query<Ciudad>(sql, new {paisid = idPais}, commandType: CommandType.StoredProcedure ).ToList();
        return resultado;
        
        // este tambien sirve en el caso de que no haya la funcion de listart
    //          public List<Ciudad> InformarCiudad(uint idPais)
    // {
